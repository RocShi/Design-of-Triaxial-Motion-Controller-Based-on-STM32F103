...
** Last Version:		V2.00
** Descriptions:		STM32F10X芯片外设头文件，适用于STM32F101/102/103/105/107芯片
**	
*********************************************************************************************************/
#ifndef _STM32F10X_H_
#define _STM32F10X_H_        //这样处理是为了避免同一个文件重复包含该头文件

#include "dtype.h"         //包含C/C++标准数据类型头文件
#include "target.h"         //包含系统初始化头文件 
	 
//用户需要根据实际芯片选择一下定义，如格兰瑞的103开发板标配STM32F103RB芯片，就选择“ #define STM32F10X_MD ”
//#define STM32F10X_LD      //小容量单片机，Flash小于等于64K的
#define STM32F10X_MD        //中容量单片机，Flash容量在128K--256K之间的
//#define STM32F10X_HD      //大容量单片机，Flash容量在256K--512K之间的
//#define STM32F10X_XL      //超大容量单片机，Flash容量在512K之上的
//#define STM32F10X_CL      //互联型单片机，指STM32F105/107系列

//配置Cortex-M3处理器和核心外设 
#ifdef STM32F10X_XL
	#define __MPU_PRESENT          1 //STM32 XL器件多出一个存储器保护单元MPU
#else
	#define __MPU_PRESENT          0 //其它器件没有
#endif //STM32F10X_XL

#define assert_param(expr)  ((void)0) //此处是定义库函数里的测试函数，这个测试函数主要是检测输入参数是否合法
                                      //一般在调试库函数时使用，我们此处定义为空函数
